...
# Option A — real login (recommended):
python -m src.auth yourpassword   # paste salt$hash into .streamlit/secrets.toml
streamlit run app.py
# Option B — local demo only (fail-open demo/demo):
#   PowerShell:  $env:STOCKFUN_DEMO="1"; streamlit run app.py
```
...
**Set it up:**
1. Generate a password hash locally (PBKDF2 + salt):
   ```bash
...
2. Add credentials to **Streamlit Cloud → app → Settings → Secrets** (or a local
   ...
   ```toml
   [auth.users]
   soumo = "salt$pbkdf2_hex_digest_from_step_1"
   ```
   Add as many `username = "salt$hash"` lines as you want.
3. **Fail-closed:** if no users are configured, the app shows setup instructions
   and will not load. For local exploration without secrets:
   ```bash
   # PowerShell
   $env:STOCKFUN_DEMO="1"; streamlit run app.py
   ```
   That enables **demo / demo** with a visible warning — never use demo mode on
   a public deploy.

Passwords are never stored or committed in plaintext (only salted PBKDF2 hashes
...
- `requirements.txt` is intentionally slim (streamlit, pandas, numpy,
  scikit-learn, pillow, joblib). LightGBM/XGBoost live in optional
  `requirements-ml.txt` for local training; yfinance is installed only when
  running the fetcher. On Cloud the model falls back to RandomForest.
- **Workflow:** run `python -m src.fetch_fundamentals` on your machine to produce
  a CSV, then upload that CSV to the live app. The fetcher is not part of the
  deployed app.

...
def main():
    ...
    # Accept common header names for the ticker column (ticker / symbol / etc.),
    # so an exported list with a "Symbol" or "Name,Symbol" header works as-is.
    ticker_aliases = ["ticker", "symbol", "tickers", "symbols", "code",
                      "nse_symbol", "yahoo_symbol"]
    found = next((c for c in ticker_aliases if c in src.columns), None)
    if found is None:
        raise SystemExit(
            "Input CSV needs a ticker column. Expected one of "
            f"{ticker_aliases}, but found columns: {list(src.columns)}. "
            "Rename your symbol column to 'ticker' (or 'symbol') and retry."
        )
    if found != "ticker":
        src = src.rename(columns={found: "ticker"})

...
def detect_red_flags(df: pd.DataFrame, by="ticker", date_col="date",
    ...
    # Aggregate (vectorized list build — avoids slow DataFrame.apply axis=1)
    flag_cols = list(flags.columns)
    if flag_cols:
        d = pd.concat([d, flags], axis=1)
        d["red_flag_count"] = d[flag_cols].fillna(False).astype(bool).sum(axis=1)
        d["red_flags"] = active_names_per_row(d, flag_cols)
    else:
